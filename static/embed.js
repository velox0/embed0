    ...
    fetch(
      ...
      .then((data) => {
        ...
        if (data.images && data.images.length > 0) {
          if (data.images.length === 1) {
            ...
            data.images.slice(0, 4).forEach(function (img) {
              imagesHtml +=
                '<div class="embed-grid-item"><img src="' +
                img +
                '" alt="' +
                data.title +
                '"></div>';
        ...
        el.innerHTML =
          ...
          (imagesHtml
            ? '<div class="embed-media">' + imagesHtml + "</div>"
            : "") +
          ...
          '<a href="' +
          data.url +
          '" target="_blank" class="embed-link">Visit website</a>' +
          "</div>" +
          "</div>" +
          "</div>";
      })
      .catch((err) => {
        el.innerHTML =
          '<p style="color:red; font-family:-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;">Embed failed >_</p>';
        console.error(err);
      });
  });
})();
